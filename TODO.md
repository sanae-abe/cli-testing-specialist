...
**最終更新**: 2025-11-10
**フェーズ**: Phase 2.6（Phase 1-5完了、実CLIテスト待ち）
...
- [x] README.md更新
  - [x] 新テストパターン追加（directory-traversal-limits）
  - [x] テスト項目12パターンの説明（機能一覧表に記載）
  - [x] セキュリティ実装ガイドラインのリンク（Support セクション）
- [x] 実行方法の明記（並列実行禁止）
  - [x] "Test Execution" セクション追加
  - [x] `bats test-output/10-directory-traversal-limits.bats`（順次実行）
  - [x] `bats --jobs 4 test-output/`（並列実行禁止の警告）
  - [x] テストカテゴリ別実行方法
  - [x] リソース集約テストのスキップ方法
- [x] トラブルシューティングセクション
  - [x] /tmp満杯時の対処法
  - [x] テスト失敗時のクリーンアップ方法
  - [x] Bash 5+要件の説明（連想配列対応）
  - [x] メモリ・リソース制限エラー対処
  - [x] BATS構文エラー説明
  - [x] i18nメッセージエラー対処
- [x] 変更履歴追加
  - [x] Phase 2.6実装完了の記録（機能一覧表）
  - [x] 新テストパターン追加の記録（README.md/README.ja.md）
- [x] README.ja.md（日本語版）更新
  - [x] テスト実行セクション追加
