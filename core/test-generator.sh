...
generate_bats_tests() {
    ...
    if [[ "$test_modules" == "all" ]] || [[ "$test_modules" == *"directory-traversal"* ]]; then
        generate_directory_traversal_tests "$analysis_json" "$validated_output_dir" && ((generated_tests++)) || true
    fi
...
#######################################
# Generate directory traversal limits tests
# Globals:
#   TEMPLATE_DIR (reads)
# Arguments:
#   $1 - Analysis JSON file path
...
generate_directory_traversal_tests() {
    ...
    log DEBUG "Generating directory traversal limits tests"
    ...
    # Detect directory traversal commands
    # Common patterns: scan, analyze, backup, sync, search, find, list, ls
    local subcommands
    subcommands=$(safe_jq '.subcommands[]?' "$analysis_json" "")

    local -a traversal_commands=()

    while IFS= read -r subcommand; do
        [[ -z "$subcommand" ]] && continue

        # Check if subcommand name suggests directory traversal operation
        if [[ "$subcommand" =~ (scan|analyze|backup|sync|search|find|list|ls|walk|tree|index) ]]; then
            traversal_commands+=("$subcommand")
            log DEBUG "Detected traversal command: $subcommand"
        fi
    done <<< "$subcommands"

    # If no traversal commands found, use generic placeholder
    if [[ ${#traversal_commands[@]} -eq 0 ]]; then
        log WARN "No traversal commands detected, using generic 'scan' as placeholder"
        traversal_commands=("scan")
    fi

    log INFO "Traversal commands: ${#traversal_commands[@]}"

    # Load directory-traversal-limits.fragment template
    local fragment_file="$TEMPLATE_DIR/directory-traversal-limits.fragment"
    if [[ ! -f "$fragment_file" ]]; then
        log ERROR "Fragment file not found: $fragment_file"
        return 1
    fi

    # Generate output file directly from fragment (no template needed)
    # directory-traversal-limits.fragment is a complete BATS file
    local output_file="$output_dir/10-directory-traversal-limits.bats"
    local first_traversal="${traversal_commands[0]}"

    # Read fragment and replace placeholders
    sed -e "s|\${TRAVERSAL_COMMAND}|$first_traversal|g" \
        -e "s|\${TRAVERSAL_ARGS}||g" \
        -e "s|\${CLI_BINARY}|$cli_binary|g" \
    ...
    log INFO "Generated: $output_file"
    log INFO "  Traversal commands tested: ${#traversal_commands[@]}"
    log INFO "  Primary command: $first_traversal"
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    ...
    if [[ $# -lt 2 ]]; then
        ...
        echo "  [test-modules]   Optional: all|basic|help|security|path|multi-shell|performance|concurrency|input-validation|destructive-ops|directory-traversal (default: all)" >&2
        echo "" >&2
        echo "Example:" >&2
